...
| Native offline basemap sources | Cache API `mapping-elf-offline-map-sources` | JSON `{ version, sources }`; source records include `format`, app-private storage path, size, checksum, and renderer status | empty index; web import disabled | no | no |

...
const PHRASES = {
  ...
  '重疊航點切換': { en: 'Overlapping waypoints', ja: '重なったウェイポイント', ko: '겹친 웨이포인트', fr: 'Waypoints superposés', de: 'Überlappende Wegpunkte', es: 'Waypoints superpuestos', it: 'Waypoint sovrapposti' },
  '地圖航點重疊時，只有雙擊航點會切換顯示層級；長按只會拖曳航點': { en: 'When map waypoints overlap, only double-clicking a waypoint switches the visible layer; long-press only drags the waypoint', ja: '地図上のウェイポイントが重なった場合、表示レイヤーの切り替えはダブルクリックのみです。長押しはドラッグだけに使います', ko: '지도 웨이포인트가 겹치면 표시 레이어 전환은 두 번 클릭으로만 가능하며, 길게 누르기는 웨이포인트 드래그에만 사용됩니다', fr: 'Quand des waypoints se chevauchent sur la carte, seul le double-clic change le niveau visible ; l’appui long sert seulement à les déplacer', de: 'Wenn Karten-Wegpunkte überlappen, wechselt nur ein Doppelklick die sichtbare Ebene; Langdruck zieht nur den Wegpunkt', es: 'Cuando se superponen waypoints en el mapa, solo el doble clic cambia la capa visible; la pulsación larga solo arrastra el waypoint', it: 'Quando i waypoint sulla mappa si sovrappongono, solo il doppio clic cambia il livello visibile; la pressione lunga trascina soltanto il waypoint' },
  ...
  '拖曳刪除': { en: 'Drag to delete', ja: 'ドラッグして削除', ko: '드래그로 삭제', fr: 'Glisser pour supprimer', de: 'Zum Löschen ziehen', es: 'Arrastra para borrar', it: 'Trascina per eliminare' },
  '地圖航點可拖到頂部': { en: 'Drag map waypoints to the top', ja: '地図上のウェイポイントは上部の', ko: '지도 웨이포인트는 상단', fr: 'Faites glisser les waypoints de carte vers la', de: 'Ziehe Karten-Wegpunkte nach oben in den', es: 'Arrastra waypoints del mapa a la', it: 'Trascina i waypoint della mappa nel' },
  '垃圾桶': { en: 'trash', ja: 'ゴミ箱', ko: '휴지통', fr: 'corbeille', de: 'Papierkorb', es: 'papelera', it: 'cestino' },
  '；側欄、表格與最愛項目可向外拖曳刪除': { en: '; drag sidebar, table, or favorite items outward to delete them', ja: 'へドラッグできます。サイドバー、表、またはお気に入り項目は外側へドラッグして削除できます', ko: '으로 끌 수 있습니다. 사이드바, 표 또는 즐겨찾기 항목은 바깥쪽으로 드래그해 삭제합니다', fr: ' ; glissez les éléments du volet, du tableau ou des favoris vers l’extérieur pour les supprimer', de: '; Seitenleisten-, Tabellen- oder Favoriteneinträge nach außen ziehen, um sie zu löschen', es: '; arrastra hacia afuera elementos de barra, tabla o favoritos para borrarlos', it: '; trascina fuori elementi laterali, tabella o preferiti per eliminarli' },
  ...
  const html = [
    ...
    `<div class="instr-group" style="margin-bottom: 16px;">
      ...
      <ul class="instr-list" style="margin: 0; padding-left: 18px; line-height: 1.6;">
        ...
        ${tipLine('重疊航點切換', '地圖航點重疊時，只有雙擊航點會切換顯示層級；長按只會拖曳航點')}
        ${tipLine('批次天氣卡', '在「航點設置」勾選連動對象後，收合、展開或關閉會套用到整組天氣卡')}
        <li><span style="font-weight: 500; color: var(--text-primary);">${t('拖曳刪除')}</span>: ${t('地圖航點可拖到頂部')} <span style="color: var(--danger);">${t('垃圾桶')}</span>${t('；側欄、表格與最愛項目可向外拖曳刪除')}</li>
      </ul>
    </div>`,
  ].join('');

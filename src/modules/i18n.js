...
const PHRASES = {
  ...
  '規劃路線後即可切換 3D': { en: 'Plan a route to switch to 3D', ja: 'ルートを作成すると3Dに切り替えられます', ko: '경로를 계획하면 3D로 전환할 수 있습니다', fr: 'Planifiez un itinéraire pour passer en 3D', de: 'Route planen, um zu 3D zu wechseln', es: 'Planifica una ruta para cambiar a 3D', it: 'Pianifica un percorso per passare al 3D' },
  '運鏡跟隨（播放時鏡頭自動跟拍人物）': { en: 'Follow camera (chases the hiker during playback)', ja: 'カメラ追従（再生中に人物を自動で追いかけます）', ko: '카메라 팔로우 (재생 중 인물을 자동 추적)', fr: 'Caméra suiveuse (suit le randonneur pendant la lecture)', de: 'Verfolgerkamera (folgt der Figur während der Wiedergabe)', es: 'Cámara de seguimiento (sigue al personaje durante la reproducción)', it: 'Telecamera a inseguimento (segue il personaggio durante la riproduzione)' },
  ...
  '副航點設置': { en: 'Secondary waypoint settings', ja: '副ウェイポイント設定', ko: '보조 웨이포인트 설정', fr: 'Réglages des waypoints secondaires', de: 'Neben-Wegpunkt-Einstellungen', es: 'Ajustes de waypoints secundarios', it: 'Impostazioni waypoint secondari' },
  ...
  '全主航點': { en: 'All main waypoints', ja: '全主要ウェイポイント', ko: '모든 주요 웨이포인트', fr: 'Tous les waypoints principaux', de: 'Alle Hauptwegpunkte', es: 'Todos los waypoints principales', it: 'Tutti i waypoint principali' },
  '全副航點': { en: 'All secondary waypoints', ja: '全副ウェイポイント', ko: '모든 보조 웨이포인트', fr: 'Tous les waypoints secondaires', de: 'Alle Neben-Wegpunkte', es: 'Todos los waypoints secundarios', it: 'Tutti i waypoint secondari' },
  ...
  '副航點': { en: 'Secondary waypoints', ja: '副ウェイポイント', ko: '보조 웨이포인트', fr: 'Waypoints secondaires', de: 'Neben-Wegpunkte', es: 'Waypoints secundarios', it: 'Waypoint secondari' },
  '詳細天氣資訊': { en: 'Detailed weather', ja: '詳細な天気', ko: '상세 날씨', fr: 'Météo détaillée', de: 'Detailliertes Wetter', es: 'Tiempo detallado', it: 'Meteo dettagliato' },
  '詳細集水區資訊': { en: 'Detailed catchment', ja: '詳細な集水域', ko: '상세 유역', fr: 'Bassin détaillé', de: 'Detailliertes Einzugsgebiet', es: 'Cuenca detallada', it: 'Bacino dettagliato' },
  '未勾選時,資訊卡只顯示切換鍵上方資訊與雨量/降雨機率/濕度/風速': { en: 'When off, the card shows only the info above the toggle plus rainfall / rain chance / humidity / wind', ja: 'オフのとき、カードは切替の上の情報と降水量／降水確率／湿度／風速のみ表示', ko: '끄면 카드에 전환 버튼 위 정보와 강수량/강수확률/습도/풍속만 표시됩니다', fr: 'Désactivé : la carte n’affiche que les infos au-dessus du sélecteur plus pluie / probabilité / humidité / vent', de: 'Aus: Die Karte zeigt nur die Infos über dem Umschalter sowie Niederschlag / -wahrscheinlichkeit / Luftfeuchte / Wind', es: 'Si se desactiva, la tarjeta solo muestra la info sobre el conmutador más lluvia / probabilidad / humedad / viento', it: 'Se disattivato, la scheda mostra solo le info sopra il selettore più pioggia / probabilità / umidità / vento' },
  '未勾選時,資訊卡集水區只顯示雨量/降雨機率/濕度/風速,不計算地形/水文': { en: 'When off, the catchment view shows only rainfall / rain chance / humidity / wind — no terrain / hydrology computed', ja: 'オフのとき、集水域ビューは降水量／降水確率／湿度／風速のみ表示し、地形／水文は計算しません', ko: '끄면 유역 보기에 강수량/강수확률/습도/풍속만 표시하고 지형/수문은 계산하지 않습니다', fr: 'Désactivé : la vue bassin n’affiche que pluie / probabilité / humidité / vent, sans calcul du relief / de l’hydrologie', de: 'Aus: Die Einzugsgebietsansicht zeigt nur Niederschlag / -wahrscheinlichkeit / Luftfeuchte / Wind — kein Gelände / keine Hydrologie', es: 'Si se desactiva, la vista de cuenca solo muestra lluvia / probabilidad / humedad / viento, sin calcular relieve / hidrología', it: 'Se disattivato, la vista bacino mostra solo pioggia / probabilità / umidità / vento, senza calcolare terreno / idrologia' },
  ...
  // Side-panel task-flow group bands
  '規劃與分析': { en: 'Plan & analyze', ja: '計画と分析', ko: '계획 및 분석', fr: 'Planification et analyse', de: 'Planen & Analysieren', es: 'Planificar y analizar', it: 'Pianifica e analizza' },
  '顯示與資料': { en: 'Display & data', ja: '表示とデータ', ko: '표시 및 데이터', fr: 'Affichage et données', de: 'Anzeige & Daten', es: 'Visualización y datos', it: 'Visualizzazione e dati' },
  '檔案': { en: 'Files', ja: 'ファイル', ko: '파일', fr: 'Fichiers', de: 'Dateien', es: 'Archivos', it: 'File' },
  '關於': { en: 'About', ja: 'について', ko: '정보', fr: 'À propos', de: 'Über', es: 'Acerca de', it: 'Informazioni' },
  // 集水區 switches — the feature's two halves, in load order (集水區域 then 水文資訊)
  '集水區域': { en: 'Catchment zone', ja: '集水域', ko: '집수 구역', fr: 'Zone du bassin', de: 'Einzugsgebiet', es: 'Zona de la cuenca', it: 'Zona del bacino' },
  '水文資訊': { en: 'Hydrology info', ja: '水文情報', ko: '수문 정보', fr: 'Infos hydrologiques', de: 'Hydrologie-Daten', es: 'Información hidrológica', it: 'Info idrologiche' },
  '集水範圍與地形數值。關閉後，量測工具、下方面板、3D 圖層與資訊卡的集水區功能都會鎖定': { en: 'The drainage zone and its terrain figures. When off, catchment is locked in the measure tool, bottom panel, 3D layer and info cards', ja: '集水範囲と地形の数値。オフにすると、計測ツール・下部パネル・3Dレイヤー・情報カードの集水域機能がロックされます', ko: '집수 범위와 지형 수치. 끄면 측정 도구, 하단 패널, 3D 레이어, 정보 카드의 집수 구역 기능이 잠깁니다', fr: 'L’étendue de drainage et ses valeurs de relief. Désactivé : le bassin versant est verrouillé dans l’outil de mesure, le panneau inférieur, la couche 3D et les fiches d’info', de: 'Der Entwässerungsumriss und seine Geländewerte. Aus: Einzugsgebiet ist im Messwerkzeug, im unteren Panel, in der 3D-Ebene und in den Infokarten gesperrt', es: 'La extensión de drenaje y sus valores de relieve. Si se desactiva, la cuenca queda bloqueada en la herramienta de medición, el panel inferior, la capa 3D y las tarjetas de info', it: 'L’estensione di drenaggio e i suoi valori di terreno. Se disattivato, il bacino è bloccato in strumento di misura, pannello inferiore, livello 3D e schede info' },
  '土壤含水量、逕流量與溪水暴漲/土石流風險等水文指標，依各欄日期時間計算。需要集水區域': { en: 'Hydrology indicators — soil moisture, runoff, flash-flood / debris-flow risk — computed for each column’s date and hour. Requires the catchment zone', ja: '土壌水分・流出量・鉄砲水／土石流リスクなどの水文指標を、各列の日時で計算します。集水域が必要です', ko: '토양 수분, 유출량, 급류/토석류 위험 등 수문 지표를 각 열의 날짜·시간 기준으로 계산합니다. 집수 구역이 필요합니다', fr: 'Indicateurs hydrologiques — humidité du sol, ruissellement, risque de crue soudaine / lave torrentielle — calculés pour la date et l’heure de chaque colonne. Nécessite la zone du bassin', de: 'Hydrologische Kennwerte — Bodenfeuchte, Abfluss, Sturzflut- / Murgang-Risiko — berechnet für Datum und Stunde jeder Spalte. Erfordert das Einzugsgebiet', es: 'Indicadores hidrológicos: humedad del suelo, escorrentía, riesgo de crecida súbita / flujo de derrubios, calculados para la fecha y hora de cada columna. Requiere la zona de la cuenca', it: 'Indicatori idrologici — umidità del suolo, deflusso, rischio di piena improvvisa / colata detritica — calcolati per data e ora di ogni colonna. Richiede la zona del bacino' },
  '集水區會讀取地形高程圖磚推算匯流範圍，資料量較大。地形結果與時間無關，算過就一直保留、不會過期，也不受天氣快取設定影響，直到按上面的清除為止。關閉後不會發出任何集水區請求，各航點的詳細集水區設定也會鎖定。': { en: 'Catchment reads elevation tiles to trace the drainage area, which is data-heavy. The terrain result is time-independent: once computed it is kept indefinitely, never expires, and ignores the weather-cache settings — until you press Clear above. When off, no catchment request is made and the per-waypoint detailed-catchment settings are locked.', ja: '集水域は標高タイルを読み込んで集水範囲を推定するため、データ量が多くなります。地形の結果は時刻に依存しないため、一度計算すれば期限切れにならず保持され、天気キャッシュ設定の影響も受けません（上の「消去」を押すまで）。オフにすると集水域のリクエストは行わず、ウェイポイントごとの詳細集水域設定もロックされます。', ko: '집수 구역은 고도 타일을 읽어 배수 범위를 계산하므로 데이터 사용량이 많습니다. 지형 결과는 시간과 무관하므로 한 번 계산하면 만료되지 않고 유지되며 날씨 캐시 설정의 영향도 받지 않습니다 (위의 지우기를 누를 때까지). 끄면 집수 구역 요청을 보내지 않고 웨이포인트별 상세 집수 구역 설정도 잠깁니다.', fr: 'Le bassin versant lit des tuiles d’altitude pour tracer la zone de drainage, ce qui consomme beaucoup de données. Le résultat du relief est indépendant du temps : une fois calculé il est conservé sans expirer et ignore les réglages du cache météo, jusqu’à ce que vous appuyiez sur Effacer ci-dessus. Désactivé, aucune requête de bassin n’est émise et les réglages de bassin détaillé par waypoint sont verrouillés.', de: 'Das Einzugsgebiet liest Höhenkacheln, um das Entwässerungsgebiet zu bestimmen — das ist datenintensiv. Das Geländeergebnis ist zeitunabhängig: einmal berechnet bleibt es erhalten, läuft nie ab und ignoriert die Wetter-Cache-Einstellungen — bis du oben auf Löschen drückst. Aus: keine Einzugsgebiets-Anfragen, und die Detail-Einstellungen je Wegpunkt sind gesperrt.', es: 'La cuenca lee teselas de elevación para trazar el área de drenaje, lo que consume muchos datos. El resultado del relieve es independiente del tiempo: una vez calculado se conserva sin caducar e ignora los ajustes de caché meteorológica, hasta que pulses Borrar arriba. Si se desactiva, no se envía ninguna solicitud de cuenca y los ajustes de cuenca detallada por waypoint quedan bloqueados.', it: 'Il bacino legge tasselli di quota per tracciare l’area di drenaggio, con un consumo dati elevato. Il risultato del terreno è indipendente dal tempo: una volta calcolato viene conservato senza scadere e ignora le impostazioni della cache meteo, finché non premi Cancella qui sopra. Se disattivato, non viene inviata alcuna richiesta e le impostazioni di bacino dettagliato per waypoint sono bloccate.' },
  '清除集水區資料': { en: 'Clear catchment data', ja: '集水域データを消去', ko: '집수 구역 데이터 지우기', fr: 'Effacer les données de bassin', de: 'Einzugsgebietsdaten löschen', es: 'Borrar datos de cuenca', it: 'Cancella dati bacino' },
  '清除已保留的集水區地形資料與各航點讀數': { en: 'Clear the retained catchment terrain data and every waypoint read-out', ja: '保持している集水域の地形データと各ウェイポイントの数値を消去します', ko: '보관 중인 집수 구역 지형 데이터와 웨이포인트별 수치를 지웁니다', fr: 'Efface les données de relief du bassin conservées et les valeurs de chaque waypoint', de: 'Löscht die gespeicherten Einzugsgebiets-Geländedaten und alle Wegpunkt-Werte', es: 'Borra los datos de relieve de cuenca conservados y las lecturas de cada waypoint', it: 'Cancella i dati di terreno del bacino conservati e le letture di ogni waypoint' },
  '已清除集水區資料': { en: 'Catchment data cleared', ja: '集水域データを消去しました', ko: '집수 구역 데이터를 지웠습니다', fr: 'Données de bassin effacées', de: 'Einzugsgebietsdaten gelöscht', es: 'Datos de cuenca borrados', it: 'Dati bacino cancellati' },
  '集水區功能已關閉': { en: 'Catchment is turned off', ja: '集水域機能はオフです', ko: '집수 구역 기능이 꺼져 있습니다', fr: 'Le bassin versant est désactivé', de: 'Einzugsgebiet ist ausgeschaltet', es: 'La cuenca está desactivada', it: 'Il bacino è disattivato' },
  '集水區功能已關閉（可在側欄「天氣設置」開啟）': { en: 'Catchment is turned off (enable it under Weather settings in the side panel)', ja: '集水域機能はオフです（サイドパネルの「天気設定」で有効化できます）', ko: '집수 구역 기능이 꺼져 있습니다 (사이드 패널의 「날씨 설정」에서 켤 수 있습니다)', fr: 'Le bassin versant est désactivé (activez-le dans « Réglages météo » du panneau latéral)', de: 'Einzugsgebiet ist ausgeschaltet (im Seitenpanel unter „Wetter-Einstellungen“ aktivierbar)', es: 'La cuenca está desactivada (actívala en «Ajustes meteorológicos» del panel lateral)', it: 'Il bacino è disattivato (attivalo in «Impostazioni meteo» nel pannello laterale)' },
  ...
  '在「副航點設置」勾選連動對象後，收合、展開或關閉會套用到整組天氣卡': { en: 'After choosing linked targets in “Secondary waypoint settings”, collapse, expand, or close applies to the whole weather-card group', ja: '「副ウェイポイント設定」で連動対象を選ぶと、折りたたみ、展開、閉じる操作が天気カード全体に適用されます', ko: '“보조 웨이포인트 설정”에서 연동 대상을 선택하면 접기, 펼치기, 닫기가 날씨 카드 그룹 전체에 적용됩니다', fr: 'Après avoir choisi les cibles liées dans « Réglages des waypoints secondaires », replier, déployer ou fermer agit sur tout le groupe météo', de: 'Nach Auswahl gekoppelter Ziele in „Neben-Wegpunkt-Einstellungen“ gilt Einklappen, Ausklappen oder Schließen für die ganze Wetterkartengruppe', es: 'Tras elegir objetivos vinculados en “Ajustes de waypoints secundarios”, plegar, desplegar o cerrar se aplica a todo el grupo', it: 'Dopo aver scelto i target in “Impostazioni waypoint secondari”, chiudi, apri o minimizza tutto il gruppo meteo' },
  ...
  '在地圖上顯示副航點的天氣圖示': { en: 'Show weather icons for secondary waypoints on the map', ja: '地図に副ウェイポイントの天気アイコンを表示', ko: '지도에 보조 웨이포인트 날씨 아이콘 표시', fr: 'Afficher les icônes météo des waypoints secondaires', de: 'Wettersymbole für Neben-Wegpunkte auf der Karte anzeigen', es: 'Mostrar iconos del tiempo de waypoints secundarios', it: 'Mostra icone meteo dei waypoint secondari' },
  ...
  '垂直軌跡圖': { en: 'Elevation', ja: '高度プロファイル', ko: '고도 프로필', fr: 'Altitude', de: 'Höhenprofil', es: 'Altitud', it: 'Altitudine' },
  '詳細天氣': { en: 'Weather', ja: '詳細天気', ko: '상세 날씨', fr: 'Météo', de: 'Wetter', es: 'Tiempo', it: 'Meteo' },
  '量測工具': { en: 'Measure tools', ja: '計測ツール', ko: '측정 도구', fr: 'Outils de mesure', de: 'Messwerkzeuge', es: 'Herramientas de medición', it: 'Strumenti di misura' },
  '軌跡量測': { en: 'Track segment', ja: 'トラック区間', ko: '트랙 구간', fr: 'Segment', de: 'Streckenabschnitt', es: 'Segmento', it: 'Segmento' },
  '距離／面積': { en: 'Distance / Area', ja: '距離／面積', ko: '거리 / 면적', fr: 'Distance / Aire', de: 'Distanz / Fläche', es: 'Distancia / Área', it: 'Distanza / Area' },
  '關閉量測': { en: 'Close measurement', ja: '計測を閉じる', ko: '측정 닫기', fr: 'Fermer la mesure', de: 'Messung schließen', es: 'Cerrar medición', it: 'Chiudi misura' },
  '點擊軌跡上兩點以量測區間距離與爬升下降': { en: 'Tap two points on the track to measure segment distance and ascent/descent', ja: 'トラック上の2点をタップして区間距離と登り／下りを計測', ko: '트랙 위 두 지점을 눌러 구간 거리와 오르내림을 측정', fr: 'Touchez deux points du tracé pour mesurer la distance et le dénivelé', de: 'Zwei Punkte auf der Strecke antippen, um Distanz und Auf-/Abstieg zu messen', es: 'Toca dos puntos de la ruta para medir distancia y desnivel', it: 'Tocca due punti sul tracciato per misurare distanza e dislivello' },
  '依序點擊地圖任意點，計算直線距離與包圍面積': { en: 'Tap points on the map in order to measure straight-line distance and enclosed area', ja: '地図上の点を順にタップして直線距離と面積を計測', ko: '지도의 지점을 순서대로 눌러 직선 거리와 면적을 측정', fr: 'Touchez des points dans l’ordre pour mesurer la distance et l’aire', de: 'Punkte der Reihe nach antippen, um Luftlinie und Fläche zu messen', es: 'Toca puntos en orden para medir distancia en línea recta y área', it: 'Tocca i punti in ordine per misurare distanza in linea retta e area' },
  '沿軌跡距離': { en: 'Along-track distance', ja: 'トラック沿い距離', ko: '트랙 거리', fr: 'Distance le long du tracé', de: 'Distanz entlang der Strecke', es: 'Distancia sobre la ruta', it: 'Distanza lungo il tracciato' },
  '直線距離': { en: 'Straight-line distance', ja: '直線距離', ko: '직선 거리', fr: 'Distance à vol d’oiseau', de: 'Luftlinie', es: 'Distancia recta', it: 'Distanza in linea retta' },
  '爬升': { en: 'Ascent', ja: '登り', ko: '오르막', fr: 'Montée', de: 'Anstieg', es: 'Ascenso', it: 'Salita' },
  '下降': { en: 'Descent', ja: '下り', ko: '내리막', fr: 'Descente', de: 'Abstieg', es: 'Descenso', it: 'Discesa' },
  '點數': { en: 'Points', ja: '点数', ko: '지점 수', fr: 'Points', de: 'Punkte', es: 'Puntos', it: 'Punti' },
  '直線總長': { en: 'Total length', ja: '合計距離', ko: '총 길이', fr: 'Longueur totale', de: 'Gesamtlänge', es: 'Longitud total', it: 'Lunghezza totale' },
  '包圍面積': { en: 'Enclosed area', ja: '囲まれた面積', ko: '둘러싼 면적', fr: 'Aire délimitée', de: 'Umschlossene Fläche', es: 'Área encerrada', it: 'Area racchiusa' },
  '提示': { en: 'Hint', ja: 'ヒント', ko: '힌트', fr: 'Indice', de: 'Hinweis', es: 'Sugerencia', it: 'Suggerimento' },
  '再點一下第二點': { en: 'Tap a second point', ja: '2点目をタップ', ko: '두 번째 지점을 누르세요', fr: 'Touchez un second point', de: 'Zweiten Punkt antippen', es: 'Toca un segundo punto', it: 'Tocca un secondo punto' },
  '先規劃路線再量測軌跡': { en: 'Plan a route first to measure the track', ja: '先にルートを計画してからトラックを計測', ko: '먼저 경로를 계획한 뒤 트랙을 측정하세요', fr: 'Planifiez d’abord un itinéraire pour mesurer le tracé', de: 'Zuerst eine Route planen, um die Strecke zu messen', es: 'Planifica una ruta primero para medir el track', it: 'Pianifica prima un percorso per misurare il tracciato' },
  '集水區': { en: 'Catchment', ja: '集水域', ko: '집수 구역', fr: 'Bassin versant', de: 'Einzugsgebiet', es: 'Cuenca', it: 'Bacino' },
  '點擊地圖任意點，繪製該處集水區範圍': { en: 'Tap any point on the map to delineate its catchment area', ja: '地図上の任意の点をタップして集水域を描画', ko: '지도의 아무 지점이나 눌러 집수 구역을 표시', fr: 'Touchez un point de la carte pour tracer son bassin versant', de: 'Beliebigen Kartenpunkt antippen, um das Einzugsgebiet zu zeichnen', es: 'Toca cualquier punto del mapa para trazar su cuenca', it: 'Tocca un punto qualsiasi della mappa per tracciare il bacino' },
  '計算集水區中…': { en: 'Computing catchment…', ja: '集水域を計算中…', ko: '집수 구역 계산 중…', fr: 'Calcul du bassin versant…', de: 'Einzugsgebiet wird berechnet…', es: 'Calculando la cuenca…', it: 'Calcolo del bacino…' },
  '此處為大面積平地，無法判定集水方向': { en: 'This spot is a large flat area — no drainage direction to trace', ja: 'ここは広い平地のため、集水方向を判定できません', ko: '이곳은 넓은 평지여서 집수 방향을 판정할 수 없습니다', fr: 'Zone plane étendue — impossible de déterminer l’écoulement', de: 'Große Ebene – keine Fließrichtung bestimmbar', es: 'Zona llana extensa: no se puede determinar el drenaje', it: 'Ampia zona pianeggiante: impossibile determinare il deflusso' },
  '海拔資料取得失敗，請稍後再試': { en: 'Failed to fetch elevation data, try again later', ja: '標高データの取得に失敗しました。後でお試しください', ko: '고도 데이터를 가져오지 못했습니다. 나중에 다시 시도하세요', fr: 'Échec de récupération des données d’altitude, réessayez plus tard', de: 'Höhendaten konnten nicht geladen werden, später erneut versuchen', es: 'No se pudieron obtener los datos de altitud, inténtalo más tarde', it: 'Impossibile recuperare i dati di quota, riprova più tardi' },
  '此處集水範圍過小': { en: 'Catchment here is too small to show', ja: 'ここの集水域は小さすぎます', ko: '이곳의 집수 구역이 너무 작습니다', fr: 'Bassin versant trop petit à cet endroit', de: 'Einzugsgebiet hier zu klein', es: 'La cuenca aquí es demasiado pequeña', it: 'Bacino troppo piccolo qui' },
  '集水區面積': { en: 'Catchment area', ja: '集水面積', ko: '집수 면적', fr: 'Surface du bassin', de: 'Einzugsfläche', es: 'Área de cuenca', it: 'Area del bacino' },
  ...
  '正在優化路線形狀...': { en: 'Refining the route shape...', ja: 'ルートの形状を最適化中...', ko: '경로 모양을 다듬는 중...', fr: 'Affinage de la forme de l’itinéraire...', de: 'Routenform wird verfeinert...', es: 'Refinando la forma de la ruta...', it: 'Perfezionamento della forma del percorso...' },
  '最大偏差': { en: 'Max deviation', ja: '最大偏差', ko: '최대 편차', fr: 'Écart max', de: 'Max. Abweichung', es: 'Desviación máx.', it: 'Deviazione max' },
  '修剪來回路段': { en: 'Trim out-and-back segments', ja: '往復セグメントを刈り込む', ko: '왕복 구간 다듬기', fr: 'Élaguer les allers-retours', de: 'Hin-und-zurück-Abschnitte kürzen', es: 'Recortar tramos de ida y vuelta', it: 'Rifila i tratti andata-ritorno' },
  '正在修剪來回路段...': { en: 'Trimming out-and-back segments...', ja: '往復セグメントを刈り込み中...', ko: '왕복 구간을 다듬는 중...', fr: 'Élagage des allers-retours...', de: 'Hin-und-zurück-Abschnitte werden gekürzt...', es: 'Recortando tramos de ida y vuelta...', it: 'Rifilatura dei tratti andata-ritorno...' },
  '來回修剪': { en: 'Out-and-back trimmed', ja: '往復刈り込み', ko: '왕복 다듬기', fr: 'Allers-retours élagués', de: 'Hin-und-zurück gekürzt', es: 'Ida y vuelta recortada', it: 'Andata-ritorno rifilato' },
  ...
  '河川流量': { en: 'River discharge', ja: '河川流量', ko: '하천 유량', fr: 'Débit du cours d’eau', de: 'Abflussmenge', es: 'Caudal fluvial', it: 'Portata fluviale' },
  '溪水暴漲風險': { en: 'Flash-flood risk', ja: '増水リスク', ko: '급류 범람 위험', fr: 'Risque de crue soudaine', de: 'Sturzflutrisiko', es: 'Riesgo de crecida súbita', it: 'Rischio di piena improvvisa' },
  '土石流風險': { en: 'Debris-flow risk', ja: '土石流リスク', ko: '토석류 위험', fr: 'Risque de lave torrentielle', de: 'Murgangrisiko', es: 'Riesgo de flujo de detritos', it: 'Rischio di colata detritica' },
  '低': { en: 'Low', ja: '低', ko: '낮음', fr: 'Faible', de: 'Gering', es: 'Bajo', it: 'Basso' },
  '中': { en: 'Moderate', ja: '中', ko: '보통', fr: 'Modéré', de: 'Mittel', es: 'Moderado', it: 'Moderato' },
  '高': { en: 'High', ja: '高', ko: '높음', fr: 'Élevé', de: 'Hoch', es: 'Alto', it: 'Alto' },
  '極高': { en: 'Very high', ja: '非常に高い', ko: '매우 높음', fr: 'Très élevé', de: 'Sehr hoch', es: 'Muy alto', it: 'Molto alto' },
  '無法評估': { en: 'N/A', ja: '評価不可', ko: '평가 불가', fr: 'Indisponible', de: 'Nicht bewertbar', es: 'No disponible', it: 'Non valutabile' },
  '水文指標為粗略估算，僅供參考': { en: 'Hydrology metrics are rough estimates, for reference only', ja: '水文指標は概算値であり、参考程度です', ko: '수문 지표는 대략적인 추정치이며 참고용입니다', fr: 'Les indicateurs hydrologiques sont des estimations approximatives, à titre indicatif seulement', de: 'Hydrologische Werte sind grobe Schätzungen, nur als Anhaltspunkt', es: 'Los indicadores hidrológicos son estimaciones aproximadas, solo de referencia', it: 'Gli indicatori idrologici sono stime approssimative, solo a titolo indicativo' },
  '載入天氣與水文中…': { en: 'Loading weather and hydrology…', ja: '気象・水文データを読み込み中…', ko: '날씨 및 수문 데이터 불러오는 중…', fr: 'Chargement météo et hydrologie…', de: 'Wetter und Hydrologie werden geladen…', es: 'Cargando meteorología e hidrología…', it: 'Caricamento meteo e idrologia…' },
  '此日期無水文資料': { en: 'No hydrology data for this date', ja: 'この日の水文データはありません', ko: '이 날짜의 수문 데이터가 없습니다', fr: 'Aucune donnée hydrologique pour cette date', de: 'Keine Hydrologiedaten für dieses Datum', es: 'Sin datos hidrológicos para esta fecha', it: 'Nessun dato idrologico per questa data' },
  '詳細集水區': { en: 'Detailed catchment', ja: '詳細集水域', ko: '상세 집수 구역', fr: 'Bassin versant détaillé', de: 'Detailliertes Einzugsgebiet', es: 'Cuenca detallada', it: 'Bacino dettagliato' },
  '取得集水區': { en: 'Get catchment', ja: '集水域を取得', ko: '집수 구역 가져오기', fr: 'Obtenir le bassin versant', de: 'Einzugsgebiet abrufen', es: 'Obtener cuenca', it: 'Ottieni bacino' },
  '正在載入集水區域': { en: 'Loading catchment zone', ja: '集水域を読み込み中', ko: '집수 구역을 불러오는 중', fr: 'Chargement de la zone du bassin', de: 'Einzugsgebiet wird geladen', es: 'Cargando zona de la cuenca', it: 'Caricamento zona bacino' },
  '正在載入水文資訊': { en: 'Loading hydrology info', ja: '水文情報を読み込み中', ko: '수문 정보를 불러오는 중', fr: 'Chargement des infos hydrologiques', de: 'Hydrologie-Daten werden geladen', es: 'Cargando información hidrológica', it: 'Caricamento info idrologiche' },
  '集水區資訊已更新': { en: 'Catchment info updated', ja: '集水域情報を更新しました', ko: '집수 구역 정보가 업데이트됨', fr: 'Infos du bassin versant mises à jour', de: 'Einzugsgebietsdaten aktualisiert', es: 'Información de la cuenca actualizada', it: 'Info bacino aggiornate' },
  '完成規劃路線後點擊「取得集水區」': { en: 'After planning a route, tap “Get catchment”', ja: 'ルート作成後に「集水域を取得」を押してください', ko: '루트 계획 후 “집수 구역 가져오기”를 누르세요', fr: 'Après avoir planifié l’itinéraire, touchez « Obtenir le bassin versant »', de: 'Nach der Routenplanung „Einzugsgebiet abrufen“ wählen', es: 'Tras planificar la ruta, toca “Obtener cuenca”', it: 'Dopo aver pianificato il percorso, tocca “Ottieni bacino”' },
  '尚未載入集水區資訊': { en: 'No catchment info loaded yet', ja: '集水域情報は未読み込みです', ko: '아직 집수 구역 정보가 없습니다', fr: 'Aucune info de bassin versant chargée', de: 'Noch keine Einzugsgebietsdaten geladen', es: 'Aún no hay información de la cuenca', it: 'Nessuna info bacino caricata' },
  ...
  '該時段出現降雨的機率百分比': { en: 'Percentage chance of rain in this time slot', ja: 'この時間帯に雨が降る確率（％）', ko: '해당 시간대에 비가 올 확률(%)', fr: 'Probabilité de pluie pour ce créneau (%)', de: 'Regenwahrscheinlichkeit für diesen Zeitraum (%)', es: 'Probabilidad de lluvia en este intervalo (%)', it: 'Probabilità di pioggia in questa fascia (%)' },
  '當日預報的最高氣溫': { en: 'Forecast high temperature for the day', ja: 'その日の予報最高気温', ko: '해당 일의 예보 최고 기온', fr: 'Température maximale prévue du jour', de: 'Vorhergesagte Tageshöchsttemperatur', es: 'Temperatura máxima prevista del día', it: 'Temperatura massima prevista del giorno' },
  '當日預報的最低氣溫': { en: 'Forecast low temperature for the day', ja: 'その日の予報最低気温', ko: '해당 일의 예보 최저 기온', fr: 'Température minimale prévue du jour', de: 'Vorhergesagte Tagestiefsttemperatur', es: 'Temperatura mínima prevista del día', it: 'Temperatura minima prevista del giorno' },
  '綜合氣溫、濕度與風速後的人體實際感受溫度': { en: 'How hot or cold it actually feels, combining temperature, humidity and wind', ja: '気温・湿度・風速を総合した体感温度', ko: '기온, 습도, 풍속을 종합한 체감 온도', fr: 'Température ressentie combinant température, humidité et vent', de: 'Gefühlte Temperatur aus Temperatur, Luftfeuchte und Wind', es: 'Sensación térmica según temperatura, humedad y viento', it: 'Temperatura percepita combinando temperatura, umidità e vento' },
  '空氣相對濕度百分比': { en: 'Relative air humidity percentage', ja: '空気の相対湿度（％）', ko: '공기의 상대 습도(%)', fr: 'Humidité relative de l’air (%)', de: 'Relative Luftfeuchtigkeit (%)', es: 'Humedad relativa del aire (%)', it: 'Umidità relativa dell’aria (%)' },
  '空氣達到飽和凝結的溫度，愈接近氣溫代表愈潮濕': { en: 'Temperature at which air saturates; the closer to air temperature, the more humid', ja: '空気が飽和して結露する温度。気温に近いほど湿っています', ko: '공기가 포화되어 응결하는 온도로, 기온에 가까울수록 습합니다', fr: 'Température de saturation de l’air ; plus elle est proche de la température, plus l’air est humide', de: 'Temperatur, bei der Luft gesättigt ist; je näher an der Lufttemperatur, desto feuchter', es: 'Temperatura de saturación del aire; cuanto más cerca de la temperatura, más húmedo', it: 'Temperatura di saturazione dell’aria; più è vicina alla temperatura, più è umido' },
  '天空被雲層覆蓋的比例': { en: 'Fraction of the sky covered by cloud', ja: '空が雲に覆われている割合', ko: '하늘이 구름으로 덮인 비율', fr: 'Proportion du ciel couverte de nuages', de: 'Anteil des von Wolken bedeckten Himmels', es: 'Proporción del cielo cubierta por nubes', it: 'Quota di cielo coperta da nuvole' },
  '該時段的平均風速': { en: 'Average wind speed for this time slot', ja: 'この時間帯の平均風速', ko: '해당 시간대의 평균 풍속', fr: 'Vitesse moyenne du vent pour ce créneau', de: 'Mittlere Windgeschwindigkeit für diesen Zeitraum', es: 'Velocidad media del viento en este intervalo', it: 'Velocità media del vento in questa fascia' },
  '該時段的最大瞬間風速': { en: 'Peak gust speed for this time slot', ja: 'この時間帯の最大瞬間風速', ko: '해당 시간대의 최대 순간 풍속', fr: 'Rafale maximale pour ce créneau', de: 'Maximale Böengeschwindigkeit für diesen Zeitraum', es: 'Ráfaga máxima en este intervalo', it: 'Raffica massima in questa fascia' },
  '紫外線指數，數值愈高愈需加強防曬': { en: 'UV index; higher values mean stronger sun protection is needed', ja: '紫外線指数。数値が高いほど日焼け対策が必要です', ko: '자외선 지수로, 값이 높을수록 자외선 차단이 필요합니다', fr: 'Indice UV ; plus il est élevé, plus la protection solaire est nécessaire', de: 'UV-Index; je höher, desto mehr Sonnenschutz nötig', es: 'Índice UV; a mayor valor, más protección solar necesaria', it: 'Indice UV; più è alto, più serve protezione solare' },
  '水平方向可看清物體的最大距離': { en: 'Maximum horizontal distance at which objects are clearly visible', ja: '水平方向で物体を視認できる最大距離', ko: '수평 방향으로 물체를 뚜렷이 볼 수 있는 최대 거리', fr: 'Distance horizontale maximale à laquelle on distingue les objets', de: 'Maximale horizontale Sichtweite für Objekte', es: 'Distancia horizontal máxima a la que se ven los objetos con claridad', it: 'Distanza orizzontale massima a cui gli oggetti sono ben visibili' },
  '當日可照射到陽光的總時數': { en: 'Total hours of sunshine for the day', ja: 'その日の日照時間の合計', ko: '해당 일의 총 일조 시간', fr: 'Nombre total d’heures d’ensoleillement du jour', de: 'Gesamte Sonnenscheindauer des Tages', es: 'Horas totales de sol del día', it: 'Ore totali di sole del giorno' },
  '太陽短波輻射強度': { en: 'Solar shortwave radiation intensity', ja: '太陽短波放射の強度', ko: '태양 단파 복사 강도', fr: 'Intensité du rayonnement solaire à ondes courtes', de: 'Intensität der kurzwelligen Solarstrahlung', es: 'Intensidad de la radiación solar de onda corta', it: 'Intensità della radiazione solare a onde corte' },
  '當日太陽升起的時刻': { en: 'Time of sunrise for the day', ja: 'その日の日の出時刻', ko: '해당 일의 일출 시각', fr: 'Heure du lever du soleil du jour', de: 'Uhrzeit des Sonnenaufgangs', es: 'Hora de salida del sol del día', it: 'Ora dell’alba del giorno' },
  '當日太陽落下的時刻': { en: 'Time of sunset for the day', ja: 'その日の日の入り時刻', ko: '해당 일의 일몰 시각', fr: 'Heure du coucher du soleil du jour', de: 'Uhrzeit des Sonnenuntergangs', es: 'Hora de puesta del sol del día', it: 'Ora del tramonto del giorno' },
  '沿路線從起點累積到此點的里程': { en: 'Cumulative distance from the start along the route to this point', ja: 'ルートに沿って起点からこの地点までの累積距離', ko: '경로를 따라 출발점부터 이 지점까지의 누적 거리', fr: 'Distance cumulée depuis le départ le long de l’itinéraire jusqu’à ce point', de: 'Aufsummierte Distanz vom Start entlang der Route bis zu diesem Punkt', es: 'Distancia acumulada desde el inicio a lo largo de la ruta hasta este punto', it: 'Distanza cumulata dalla partenza lungo il percorso fino a questo punto' },
  '此點地面的高程': { en: 'Ground elevation at this point', ja: 'この地点の地面の標高', ko: '이 지점의 지면 고도', fr: 'Altitude du sol à ce point', de: 'Geländehöhe an diesem Punkt', es: 'Altitud del terreno en este punto', it: 'Quota del terreno in questo punto' },
  '此點的經緯度座標（點擊可複製）': { en: 'Latitude/longitude of this point (click to copy)', ja: 'この地点の緯度・経度（クリックでコピー）', ko: '이 지점의 위도·경도 (클릭하여 복사)', fr: 'Latitude/longitude de ce point (cliquer pour copier)', de: 'Breiten-/Längengrad dieses Punktes (zum Kopieren klicken)', es: 'Latitud/longitud de este punto (clic para copiar)', it: 'Latitudine/longitudine di questo punto (clic per copiare)' },
  '雨水最終匯流至此出水口的地表範圍面積': { en: 'Surface area whose rainfall ultimately drains to this outlet', ja: '雨水が最終的にこの流出口へ集まる地表の面積', ko: '빗물이 최종적으로 이 유출구로 모이는 지표 면적', fr: 'Superficie dont les eaux de pluie s’écoulent finalement vers cet exutoire', de: 'Fläche, deren Niederschlag letztlich zu diesem Auslass abfließt', es: 'Superficie cuya lluvia drena finalmente hacia este desagüe', it: 'Superficie la cui pioggia confluisce infine verso questo sbocco' },
  '集水區最低匯流點（出水口）的高程': { en: 'Elevation of the catchment’s lowest drainage point (outlet)', ja: '集水域の最低流出点（流出口）の標高', ko: '집수 구역의 최저 배수 지점(유출구) 고도', fr: 'Altitude du point de drainage le plus bas du bassin (exutoire)', de: 'Höhe des tiefsten Abflusspunkts des Einzugsgebiets (Auslass)', es: 'Altitud del punto de drenaje más bajo de la cuenca (desagüe)', it: 'Quota del punto di deflusso più basso del bacino (sbocco)' },
  '集水區內最低到最高的高程差': { en: 'Elevation difference from the lowest to the highest point in the catchment', ja: '集水域内の最低点から最高点までの標高差', ko: '집수 구역 내 최저점부터 최고점까지의 고도 차', fr: 'Dénivelé entre le point le plus bas et le plus haut du bassin', de: 'Höhenunterschied zwischen tiefstem und höchstem Punkt im Einzugsgebiet', es: 'Diferencia de altitud entre el punto más bajo y el más alto de la cuenca', it: 'Dislivello tra il punto più basso e quello più alto del bacino' },
  '集水區地表的平均傾斜角度': { en: 'Average slope angle of the catchment terrain', ja: '集水域地表の平均傾斜角', ko: '집수 구역 지표의 평균 경사각', fr: 'Angle de pente moyen du terrain du bassin', de: 'Mittlerer Neigungswinkel des Einzugsgebiets', es: 'Ángulo de pendiente medio del terreno de la cuenca', it: 'Pendenza media del terreno del bacino' },
  '集水區內最長匯流路徑（主河道）的長度': { en: 'Length of the longest drainage path (main channel) in the catchment', ja: '集水域内で最長の流路（本流）の長さ', ko: '집수 구역 내 가장 긴 유로(본류)의 길이', fr: 'Longueur du plus long chemin d’écoulement (cours principal) du bassin', de: 'Länge des längsten Fließwegs (Hauptgerinne) im Einzugsgebiet', es: 'Longitud del cauce de drenaje más largo (canal principal) de la cuenca', it: 'Lunghezza del percorso di deflusso più lungo (asta principale) del bacino' },
  '根區土壤的水分飽和百分比，愈高愈易形成逕流': { en: 'Root-zone soil moisture saturation; higher values make runoff more likely', ja: '根圏土壌の水分飽和度。高いほど表面流出が起きやすくなります', ko: '뿌리 영역 토양의 수분 포화도로, 높을수록 유출이 잘 발생합니다', fr: 'Saturation en eau du sol dans la zone racinaire ; plus elle est élevée, plus le ruissellement est probable', de: 'Bodenfeuchtesättigung im Wurzelbereich; höhere Werte begünstigen Abfluss', es: 'Saturación de humedad del suelo en la zona radicular; a mayor valor, más escorrentía', it: 'Saturazione idrica del suolo nella zona radicale; più è alta, più facilmente si forma deflusso' },
  '事件前 24／72 小時的累積雨量，反映地表既有濕度': { en: 'Rainfall accumulated in the 24/72 h before the event, indicating existing ground wetness', ja: 'イベント前24／72時間の積算雨量。地表の既存の湿り具合を反映します', ko: '사건 전 24／72시간 누적 강우량으로, 지표의 기존 습윤 상태를 반영합니다', fr: 'Pluie cumulée dans les 24/72 h précédant l’événement, reflétant l’humidité déjà présente du sol', de: 'Niederschlag der 24/72 h vor dem Ereignis, zeigt die bereits vorhandene Bodennässe', es: 'Lluvia acumulada en las 24/72 h previas al evento, refleja la humedad ya existente del suelo', it: 'Pioggia cumulata nelle 24/72 h prima dell’evento, indica l’umidità già presente del suolo' },
  '依 SCS-CN 法估算此次降雨形成的地表逕流體積': { en: 'Surface runoff volume from this rainfall, estimated with the SCS-CN method', ja: 'SCS-CN法で推定した今回の降雨による表面流出量', ko: 'SCS-CN 방법으로 추정한 이번 강우의 지표 유출량', fr: 'Volume de ruissellement de cette pluie, estimé par la méthode SCS-CN', de: 'Oberflächenabfluss dieses Niederschlags, geschätzt nach der SCS-CN-Methode', es: 'Volumen de escorrentía de esta lluvia, estimado con el método SCS-CN', it: 'Volume di deflusso superficiale di questa pioggia, stimato con il metodo SCS-CN' },
  '區域河道的模擬流量，以及相對於常態的距平': { en: 'Simulated river discharge for the area and its anomaly versus normal', ja: '地域河川の模擬流量と、平年からの偏差', ko: '지역 하천의 모의 유량과 평년 대비 편차', fr: 'Débit simulé du cours d’eau régional et son écart par rapport à la normale', de: 'Simulierter Abfluss des regionalen Gewässers und Abweichung vom Normalwert', es: 'Caudal simulado del cauce regional y su anomalía respecto a lo normal', it: 'Portata simulata del corso d’acqua regionale e la sua anomalia rispetto alla norma' },
  '綜合降雨強度、土壤濕度與地形研判的溪水暴漲參考等級（非官方警報）': { en: 'Reference flash-flood level from rain intensity, soil moisture and terrain (not an official alert)', ja: '降雨強度・土壌水分・地形から判断した増水の参考レベル（公式警報ではありません）', ko: '강우 강도, 토양 습도, 지형을 종합한 급류 참고 등급 (공식 경보 아님)', fr: 'Niveau indicatif de crue soudaine d’après l’intensité de pluie, l’humidité du sol et le relief (pas une alerte officielle)', de: 'Richtwert für Sturzfluten aus Regenintensität, Bodenfeuchte und Gelände (keine amtliche Warnung)', es: 'Nivel de referencia de crecida súbita según intensidad de lluvia, humedad del suelo y relieve (no es un aviso oficial)', it: 'Livello indicativo di piena improvvisa da intensità di pioggia, umidità del suolo e morfologia (non è un’allerta ufficiale)' },
  '依降雨強度—延時門檻與坡度、濕度研判的土石流參考等級（非官方警報）': { en: 'Reference debris-flow level from rainfall intensity-duration thresholds, slope and moisture (not an official alert)', ja: '降雨強度・継続時間の閾値と傾斜・湿度から判断した土石流の参考レベル（公式警報ではありません）', ko: '강우 강도–지속시간 임계값과 경사, 습도를 종합한 토석류 참고 등급 (공식 경보 아님)', fr: 'Niveau indicatif de lave torrentielle d’après les seuils intensité-durée de pluie, la pente et l’humidité (pas une alerte officielle)', de: 'Richtwert für Murgänge aus Regen-Intensitäts-Dauer-Schwellen, Hangneigung und Feuchte (keine amtliche Warnung)', es: 'Nivel de referencia de flujo de detritos según umbrales intensidad-duración de lluvia, pendiente y humedad (no es un aviso oficial)', it: 'Livello indicativo di colata detritica da soglie intensità-durata di pioggia, pendenza e umidità (non è un’allerta ufficiale)' },
  '時間': { en: 'Time', ja: '時刻', ko: '시간', fr: 'Heure', de: 'Zeit', es: 'Hora', it: 'Ora' },
  '今日': { en: 'Today', ja: '今日', ko: '오늘', fr: 'Aujourd’hui', de: 'Heute', es: 'Hoy', it: 'Oggi' },
  '現在': { en: 'Now', ja: '現在', ko: '지금', fr: 'Maintenant', de: 'Jetzt', es: 'Ahora', it: 'Ora' },
  '設為今日': { en: 'Set to today', ja: '今日に設定', ko: '오늘로 설정', fr: 'Régler sur aujourd’hui', de: 'Auf heute setzen', es: 'Establecer a hoy', it: 'Imposta a oggi' },
  '設為現在': { en: 'Set to now', ja: '現在時刻に設定', ko: '지금으로 설정', fr: 'Régler sur maintenant', de: 'Auf jetzt setzen', es: 'Establecer a ahora', it: 'Imposta ad ora' },
  ...
  '取得離線底圖': { en: 'Get offline base maps', ja: 'オフライン地図を入手', ko: '오프라인 지도 받기', fr: 'Obtenir des cartes hors ligne', de: 'Offline-Karten holen', es: 'Obtener mapas offline', it: 'Ottieni mappe offline' },
  '下載 .map 圖資後,用下方「匯入底圖」載入': { en: 'After downloading a .map file, load it with "Import base map" below', ja: '.map ファイルをダウンロード後、下の「地図をインポート」で読み込みます', ko: '.map 파일을 내려받은 뒤 아래 "베이스맵 가져오기"로 불러오세요', fr: 'Après avoir téléchargé un fichier .map, chargez-le avec « Importer la carte » ci-dessous', de: 'Nach dem Download einer .map-Datei unten mit „Karte importieren“ laden', es: 'Tras descargar un archivo .map, cárgalo con "Importar mapa base" abajo', it: 'Dopo aver scaricato un file .map, caricalo con "Importa mappa" qui sotto' },
  '魯地圖': { en: 'Rudy Map', ja: 'Rudy Map', ko: 'Rudy Map', fr: 'Rudy Map', de: 'Rudy Map', es: 'Rudy Map', it: 'Rudy Map' },
  '台灣': { en: 'Taiwan', ja: '台湾', ko: '대만', fr: 'Taïwan', de: 'Taiwan', es: 'Taiwán', it: 'Taiwan' },
  '世界': { en: 'World', ja: '世界', ko: '세계', fr: 'Monde', de: 'Welt', es: 'Mundo', it: 'Mondo' },
  '下載魯地圖離線圖資（台灣）': { en: 'Download Rudy Map offline maps (Taiwan)', ja: 'Rudy Map のオフライン地図をダウンロード（台湾）', ko: 'Rudy Map 오프라인 지도 다운로드 (대만)', fr: 'Télécharger les cartes hors ligne Rudy Map (Taïwan)', de: 'Rudy Map Offline-Karten herunterladen (Taiwan)', es: 'Descargar mapas offline de Rudy Map (Taiwán)', it: 'Scarica le mappe offline di Rudy Map (Taiwan)' },
  '下載 OpenAndroMaps 離線圖資（世界地圖）': { en: 'Download OpenAndroMaps offline maps (worldwide)', ja: 'OpenAndroMaps のオフライン地図をダウンロード（世界）', ko: 'OpenAndroMaps 오프라인 지도 다운로드 (세계)', fr: 'Télécharger les cartes hors ligne OpenAndroMaps (monde)', de: 'OpenAndroMaps Offline-Karten herunterladen (weltweit)', es: 'Descargar mapas offline de OpenAndroMaps (mundial)', it: 'Scarica le mappe offline di OpenAndroMaps (mondiale)' },
  '開啟下載頁面失敗': { en: 'Failed to open download page', ja: 'ダウンロードページを開けませんでした', ko: '다운로드 페이지를 열지 못했습니다', fr: 'Impossible d’ouvrir la page de téléchargement', de: 'Download-Seite konnte nicht geöffnet werden', es: 'No se pudo abrir la página de descarga', it: 'Impossibile aprire la pagina di download' },
  ...
  '未選取任何操作目標 (主航點 / 副航點)': { en: 'No targets selected (main / secondary waypoints)', ja: '対象が選択されていません（主要 / 副ウェイポイント）', ko: '선택된 대상이 없습니다 (주요 / 보조 웨이포인트)', fr: 'Aucune cible sélectionnée (waypoints principaux / secondaires)', de: 'Keine Ziele ausgewählt (Haupt- / Neben-Wegpunkte)', es: 'No hay objetivos seleccionados (principales / secundarios)', it: 'Nessun obiettivo selezionato (principali / secondari)' },
  ...
  const html = [
    ...
    `<div class="instr-group" style="margin-bottom: 16px;">
      ...
      <ul class="instr-list" style="margin: 0; padding-left: 18px; line-height: 1.6;">
        ...
        ${tipLine('批次天氣卡', '在「副航點設置」勾選連動對象後，收合、展開或關閉會套用到整組天氣卡')}
...
function translatePattern(text) {
  ...
  m = text.match(/^副航點(?:\s+(\d+))?$/);
  if (m) return withNum(m[1] ? 'Secondary waypoint {n}' : 'Secondary waypoint', m[1] ? '副ウェイポイント {n}' : '副ウェイポイント', m[1] ? '보조 웨이포인트 {n}' : '보조 웨이포인트', m[1] ? 'Waypoint secondaire {n}' : 'Waypoint secondaire', m[1] ? 'Neben-Wegpunkt {n}' : 'Neben-Wegpunkt', m[1] ? 'Waypoint secundario {n}' : 'Waypoint secundario', m[1] ? 'Waypoint secondario {n}' : 'Waypoint secondario', m[1]);
  // Legacy label kept for stored waypoints/.melmap saved before the 中繼點→副航點 rename.

...
| P2 | 完成 | 保存與分享整併 | 將 GPX/KML 匯入匯出、`.melmap`、離線圖磚與我的最愛收斂成「路線庫 / 保存與分享」流程。 | 2026-05-24 完成第二階段；單一入口可保存、匯入、保存與分享、管理離線包，且路線規劃 header 不再重複放保存/分享入口；既有 `.melmap` 與 favorites 契約維持相容。 |
...
- 影響範圍：路線規劃 header 移除「保存到路線庫 / 打開路線庫」重複按鈕，只保留復原、重做、重新規劃、反向重規劃與清除等路線編輯操作。路線庫 header 移除第二組保存/匯入/匯出快捷鍵，保存、匯入與保存與分享集中到路線庫內容的主操作列；舊「已保存路線」modal 與長按拖曳刪除分支移除，由路線庫 inline 載入/刪除取代。

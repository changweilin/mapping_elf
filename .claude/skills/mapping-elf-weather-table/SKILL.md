...
- **Round-trip interval points**: In speed/km mode, `computeHourlyPoints`/`computeIntermediatePoints` process the full round-trip route and produce interval points for BOTH legs. `buildWeatherPoints` marks any non-waypoint with `_cum > oneWayDistM` as `isReturn: true`. In fallback mode, return midpoints are added by mirroring each outbound midpoint: `returnCum = totalDistM - pt._cum`, `_elapsedH = getElapsedH(returnCumMid)`.
- **`firstReturnIdx`**: Relies on all return-leg points (including interval) being marked `isReturn: true`. Without this, the midline separator lands on the first return *waypoint* instead of right after the turnaround.
